package com.hxs.valentineluckdraw.weight;
...
public class LuckPan extends View {
    ...
    public void startAnim() {
        mLuckNum = new Random().nextInt(mItemStrs.length);//随机生成结束位置
    ...
    private void drawPanItem(Canvas canvas) {
        ...
        if (mItemStrs.length % 2 == 0) {
            for (int x = 1; x <= mItemStrs.length; x++) {
                if (x % 2 == 1) {
                    //是奇数
                    mPaintArc.setColor(Color.parseColor("#008577"));
                } else {
                    //偶数
            ...
            for (int x = 1; x <= mItemStrs.length; x++) {
                if (x % 3 == 2) {
                    ...
                    mPaintArc.setColor(Color.parseColor("#4A184C"));
                } else {
                    mPaintArc.setColor(Color.parseColor("#F8864A"));
                }
                Path path = new Path();
